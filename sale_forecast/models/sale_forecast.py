...
class sale_forecast(models.Model):
    ...
    @api.v7
    def onchange_forecast_filter(self, ids, forecast_filter_id, context=None):
        f_period_obj = self.env['forecast.period']
        f_product_obj = self.env['forecast.product']
        if not ids:
            return {}
        if forecast_filter_id:
            s_rec = f_product_obj.search([('forecast_id', '=', ids[0]), (
            'period_start_date', '=', f_period_obj.browse(forecast_filter_id, context).p_date)])

        else:
            s_rec = f_product_obj.search([('forecast_id', '=', ids[0])], context)
    ...
    @api.v7
    def onchange_required_process(self, ids, required_process, context=None):
        ...
        if not required_process:
            return {}
        if required_process:
            forecast_rec = self.browse(ids, context)
        ...
        self.write(ids[0], {'forecast_product_ids': line_list})
        return {'value': {'forecast_product_ids': line_list}}

    @api.v7
    def onchange_sales_person_id(self, ids, sales_person_id, context=None):
        ...
        self.write(ids[0], {'forecast_product_ids': line_sales_person})
    ...
    def generate_forecast(self):
        ...
        for index, item in enumerate(periods):
            ...
            for product in product_ids:
                ...
                #                     self.action = 'manufacture'
                # else:
                #     self.action = 'buy'
                ctx = self._context.copy()
                ctx.update(
                    {'from_date': item.name, 'to_date': end_date, 'warehouse': self.warehouse_id.id, 'rec_id': self.id})
                product_qty = product._product_available()
                print (product_qty)
                qty_list = product_qty.get(product.id, None)
                last_period_qty, sec_last_period_qty, third_last_period_qty, avg_qty = 0.0, 0.0, 0.0, 0.0
                if first_period == True and qty_list:
...
class product_product(models.Model):
    ...
    forcast_id = fields.Many2one('sale.forecast', string='Forecast')
